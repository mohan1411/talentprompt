...
def get_email_service():
    ...
    if all([settings.SMTP_HOST, settings.SMTP_USER, settings.SMTP_PASSWORD]):
        print(f"[EMAIL] SMTP configured - Host: {settings.SMTP_HOST}, User: {settings.SMTP_USER}", flush=True)
        sys.stdout.flush()
        ...
        try:
            ...
            print(f"[EMAIL] Failed to import SMTP service: {e}", flush=True)
            sys.stdout.flush()
            logger.error(f"Failed to import SMTP email service: {e}")
            logger.warning("Falling back to mock email service")
    else:
        print(f"[EMAIL] SMTP not configured - Using mock email service", flush=True)
        print(f"[EMAIL] SMTP_HOST: {settings.SMTP_HOST or 'Not set'}", flush=True)
        print(f"[EMAIL] SMTP_USER: {settings.SMTP_USER or 'Not set'}", flush=True)
        print(f"[EMAIL] SMTP_PASSWORD: {'Set' if settings.SMTP_PASSWORD else 'Not set'}", flush=True)
        sys.stdout.flush()
