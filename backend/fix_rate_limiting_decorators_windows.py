#!/usr/bin/env python3
"""
Fix rate limiting decorators to include request parameter (Windows compatible)
"""
...
def fix_auth_endpoints():
    ...
    if not os.path.exists(auth_py):
        print(f"[ERROR] {auth_py} not found")
        return False
    ...
    if "from fastapi import" in content and ", Request" not in content:
        ...
        print("[OK] Added Request to imports")
    ...
    for endpoint in endpoints_to_fix:
        if endpoint['old'] in content:
            ...
            print(f"[OK] Fixed {endpoint['name']} endpoint")
        else:
            ...
            if new_content != content:
                content = new_content
                print(f"[OK] Fixed {endpoint['name']} endpoint (pattern match)")
...
print("[OK] Rate limiting temporarily disabled")
'''
    
    with open("disable_rate_limiting.py", "w", encoding='utf-8') as f:
        f.write(simple_fix)
    
    print("[OK] Created disable_rate_limiting.py as a fallback option")
...
def main():
    ...
    if success:
        print("\n[SUCCESS] Rate limiting decorators fixed!")
