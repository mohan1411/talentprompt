#!/usr/bin/env python3
"""Generate test resumes for bulk upload testing - simplified version."""
...
def main():
    ...
    # Generate resumes (only TXT format for simplicity)
    resume_count = 0
    for position, count in position_counts.items():
        for i in range(count):
            ...
            # Generate filename
            filename = f"{resume_data['first_name']}_{resume_data['last_name']}_{position.replace(' ', '_')}_{resume_count}.txt"
    ...
    print("\n✅ Successfully generated 50 test resumes!")
    print(f"📁 Individual resumes: {individual_dir}")
    print(f"📦 Bulk ZIP files: {bulk_dir}")
    print("\nZIP files created:")
    print("- all_resumes.zip (50 resumes)")
    print("- Position-specific ZIPs (6 files)")
    print("- Mixed batches (5 files with 10 resumes each)")
