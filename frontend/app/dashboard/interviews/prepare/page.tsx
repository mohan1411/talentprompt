...
export default function PrepareInterviewPage() {
  ...
  const [pipelineStateId, setPipelineStateId] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    const pipelineStateIdParam = searchParams.get('pipeline_state_id')
    
    // Handle pipeline context from sessionStorage
    const contextStr = sessionStorage.getItem('interviewPipelineContext')
    if (contextStr) {
      try {
        const context = JSON.parse(contextStr)
        setPipelineContext(context)
        sessionStorage.removeItem('interviewPipelineContext')
      } catch (e) {
        console.error('Failed to parse pipeline context:', e)
      }
    }
    
    if (pipelineStateIdParam) {
      setPipelineStateId(pipelineStateIdParam)
    }
  ...
  const handlePrepareInterview = async () => {
    ...
    try {
      console.log('Preparing interview with resume:', selectedResume.id, 'position:', jobPosition, 'pipeline_state_id:', pipelineStateId)
      ...
      const response = await interviewsApi.prepareInterview({
        ...
        pipeline_state_id: pipelineStateId || undefined,  // Link to pipeline if coming from pipeline view
      ...
      <div className="mb-6">
        ...
        {pipelineContext && (
          <div className="mt-3 inline-flex items-center gap-2 px-3 py-1 bg-blue-50 text-blue-700 rounded-lg text-sm">
            ...
            <span>Linked to pipeline: {pipelineContext.candidateName} (Stage: {pipelineContext.currentStage})</span>
          </div>
        )}
      </div>
